...
class LLMClient(ABC):
    ...
    @abstractmethod
    def generate_batch(
        self, inputs: dict[str, list], return_exceptions: typing.Literal[True] = True
    ) -> list[str | BaseException]: ...

    @typing.overload
    @abstractmethod
    def generate_batch(
        self, inputs: dict[str, list], return_exceptions: typing.Literal[False] = False
    ) -> list[str]: ...
...
class LLMService(ABC):
    ...
    ) -> "morpheus.llm.services.nemo_llm_service.OpenAILLMService":
        pass
    ...
    def create(
        ...
        **service_kwargs,
        ...
        """
        module_name = f"src.{service_type.lower()}_llm_service"
        ...
        ), f"Expected to find exactly one class with name {class_name_lower} in module {module_name}, but found {matching_classes}"
