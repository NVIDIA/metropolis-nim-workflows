...
class OpenAIChatClient(LLMClient):
    ...
    def _generate(
        ...
        prompt: str,
        assistant: str = None,
        return_exceptions: typing.Literal[False] = False,
    ) -> str: ...

    def _generate(
        self, prompt: str, assistant: str = None, return_exceptions: bool = False
    ):
    ...
    async def generate_async(self, **input_dict) -> str:
        ...
        return await self._generate_async(
            input_dict[self._prompt_key], input_dict.get(self._assistant_key)
        )
    ...
    def generate_batch(
        ...
        for i, prompt in enumerate(prompts):
            ...
            if return_exceptions:
                results.append(
                    self._generate(prompt, assistant, return_exceptions=True, **kwargs)
                )
            else:
                results.append(
                    self._generate(prompt, assistant, return_exceptions=False, **kwargs)
                )
    ...
    async def generate_batch_async(
        ...
        return await asyncio.gather(
            *coros, return_exceptions=return_exceptions, **kwargs
        )
